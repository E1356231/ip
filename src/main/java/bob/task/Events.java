...
public class Events extends Task {
    ...
    public String toFileString() {
        return TYPE + " | " + super.toFileString() + " | " + this.startDate + ", " + this.startTime
    ...
    public static Events fromFileString(String line) {
        String[] parts = line.split(" \\| ", 5); // T | status | description | start datetime | end datetime
        String[] start = parts[3].split(", ", 2);
        String[] end = parts[4].split(", ", 2);
        Events e = new Events(parts[2], start[0], start[1], end[0], end[1]);
        if (parts[1].equals("1")) {
            e.markDone();
        }
        return e;
